...
describe("ChunkedAudioPlayer", () => {
  ...
  async function makePlayer(chunks: AudioChunk[][] = [makeChunks()]): Promise<ChunkedAudioPlayer> {
    return ChunkedAudioPlayer.create(ctx, chunks, {
      tracks: chunks.map(() => ({})),
      onAmplitudes: () => {},
    });
  }
  ...
  it("create() resolves to a ChunkedAudioPlayer instance", async () => {
  ...
  it("scheduleSeek into a silent gap between chunks does not throw", async () => {
    ...
    const player = await ChunkedAudioPlayer.create(ctx, chunks, {
      tracks: [{}],
      onAmplitudes: () => {},
    });
    player.play();
  ...
  it("passes a mono chunk signal through to the output", async () => {
    ...
    const player = await ChunkedAudioPlayer.create(
